...
<div style="background: #f0f2fa; min-height: 100vh; padding: 32px 24px; display: flex; justify-content: center;">
  <div style="background: #fff; border-radius: 16px; border: 1px solid #e5e7eb; padding: 32px; width: 100%; max-width: 640px; height: fit-content;">
    ...
    <div style="margin-bottom: 28px; padding-bottom: 20px; border-bottom: 1px solid #f3f4f6;">
      <h2 style="font-size: 20px; font-weight: 600; color: #1a1a2e;">Create Job Posting</h2>
    ...
    <form action="{{ route('jobs.store') }}" method="POST">
      ...
      <div style="margin-bottom: 18px;">
        ...
        <textarea name="description" id="description" rows="5" required
          ...
          style="width:100%; padding:9px 12px; border-radius:8px; border:1px solid {{ $errors->has('description') ? '#A32D2D' : '#d1d5db' }}; font-size:14px; font-family:inherit; resize:vertical; outline:none;">{{ old('description') }}</textarea>
        @error('description')
          <p style="font-size:12px; color:#A32D2D; margin-top:4px;">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div style="margin-bottom: 18px;">
        ...
        <textarea name="requirements" id="requirements" rows="4"
          ...
          style="width:100%; padding:9px 12px; border-radius:8px; border:1px solid #d1d5db; font-size:14px; font-family:inherit; resize:vertical; outline:none;">{{ old('requirements') }}</textarea>
      </div>
      ...
      <div style="margin-bottom: 24px;">
        ...
        <select name="status" id="status"
          ...
          <option value="open"   {{ old('status', 'open') == 'open'   ? 'selected' : '' }}>Open — visible to applicants</option>
          <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>Closed — hidden from listings</option>
        </select>
      </div>
